...
def send_slack_briefing(
    ...
    Args:
        ...
        local_path: Optional path to local Markdown briefing file
    ...
    try:
        ...
        context_text = f"{len(papers)} papers · {today}"
        if stats:
            total = stats.get("total_fetched")
            dedup = stats.get("after_dedup")
            if total and dedup:
                context_text += f" · {total} fetched → {dedup} after dedup"

        blocks = [
            {
                "type": "header",
                "text": {"type": "plain_text", "text": "📚 Daily Paper Briefing"}
            },
            {
                "type": "context",
        ...
        # Footer
        footer_text = "Generated by AtomOra 🔬"
        if local_path:
            footer_text += f"  ·  `{local_path}`"
        blocks.append({
            "type": "context",
        ...
        print(f"✓ Sent briefing to Slack ({len(papers)} papers)")
        return True

    except Exception as e:
        print(f"✗ Failed to send Slack briefing: {e}")
        return False
